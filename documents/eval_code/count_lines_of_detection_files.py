...
#This is a threshold for how far off the number of lines in a detection file can be from the number of known points in its ground truth file, smaller threshold
#values mean detection files musts have a more similar amount detections to the known number of points
the_thresh = 0.03


# For each setting folder ittterate through rs and fq detection files. Append all to list whos line number is within some number of the known ground truth points.
# Repeat this across all settings directories and you end up with text files that list the files names of at least plausible detection files,
# its silly and time consuming to analyze a file with 10,000 detections when you know the ground truth has 300.
